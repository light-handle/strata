...
export default function ConversationModal({ sessionId, onClose }: Props) {
  ...
  const visibleBlocks = data ? data.blocks.slice(0, visibleCount) : []

  // Build entries with time gaps
  ...
  for (let i = 0; i < visibleBlocks.length; i++) {
    ...
    if (i > 0) {
      ...
      const gap = new Date(block.timestamp).getTime() - new Date(prev.timestamp).getTime()
      ...
      >
            ...
            {data && (
              <div className="flex items-center gap-4 text-[13px]">
                <span className="text-text-muted">
                  <span className="text-primary font-semibold">{data.blocks.filter((b) => b.type === 'user-prompt').length}</span> prompts
                </span>
                <span className="text-text-muted">
                  <span className="text-secondary font-semibold">{data.blocks.filter((b) => b.type === 'tool-use').length}</span> tool calls
                </span>
                <span className="text-text-muted">
                  <span className="text-text-bright font-semibold">{data.blocks.length}</span> blocks
                </span>
                {data.subagents.length > 0 && (
                  <span className="text-text-muted">
                    <span className="text-secondary font-semibold">{data.subagents.length}</span> subagents
                  </span>
                )}
              </div>
            )}
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto min-h-0 py-3">
          ...
          {data && data.blocks.length === 0 && (
            <div className="flex items-center justify-center py-16">
              <div className="text-[13px] text-text-muted">No conversation data found.</div>
            </div>
          )}
...
// ── Block components (sized for the wide modal) ──
    ...
    <div className="mx-6 my-1 rounded-lg" style={{ borderLeft: '3px solid var(--color-secondary)', background: 'rgba(0,229,255,0.03)' }}>
      <div className="px-5 py-3">
        ...
        <div className="text-[13px] text-text leading-relaxed mt-1.5 whitespace-pre-wrap break-words">
          {display}
          {!expanded && (
            <button onClick={() => setExpanded(true)} className="text-secondary text-[12px] ml-1 hover:underline">
              show more...
            </button>
          )}
        </div>
...
function ThinkingBlockWide({ text }: { text: string }) {
  ...
  return (
    <div
      className="mx-6 my-1 rounded-lg cursor-pointer"
      style={{ borderLeft: '3px solid rgba(136,102,204,0.5)', background: 'rgba(136,102,204,0.04)' }}
      ...
      <div className="px-5 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-text-muted ml-auto">
            {expanded ? '&#9660;' : '&#9654;'} ~{Math.round(text.length / 5)} words
          </span>
        </div>
        {expanded && (
          <div className="mt-2 text-[12px] text-text-muted leading-relaxed whitespace-pre-wrap break-words overflow-y-auto" style={{ maxHeight: '300px' }}>
            {text}
          </div>
        )}
      </div>
...
function ToolUseBlock({ name, input }: { name: string; input: Record<string, unknown> }) {
  ...
  return (
    <div
      className="mx-6 my-1 rounded-lg cursor-pointer"
      style={{ background: 'rgba(8,8,12,0.8)', border: '1px solid rgba(255,180,50,0.08)' }}
      onClick={() => setExpanded(!expanded)}
    >
      <div className="px-5 py-2.5">
        <div className="flex items-center gap-3">
        ...
        {expanded && (
          <pre className="mt-2 text-[11px] text-text leading-relaxed overflow-auto rounded-lg p-4"
            style={{ background: 'rgba(0,0,0,0.4)', whiteSpace: 'pre-wrap', wordBreak: 'break-all', maxHeight: '250px' }}>
...
function ToolResultBlockWide({ content, isError }: { content: string; isError: boolean }) {
  ...
  return (
    <div
      className="mx-6 -mt-0.5 mb-1 rounded-lg cursor-pointer"
      style={{
        background: isError ? 'rgba(255,68,102,0.06)' : 'rgba(8,8,12,0.5)',
        borderLeft: `3px solid ${isError ? 'rgba(255,68,102,0.5)' : 'rgba(255,180,50,0.1)'}`,
      }}
      onClick={() => setExpanded(!expanded)}
    >
      <div className="px-5 py-2">
        <div className="flex items-center gap-2">
          <span className={`text-[11px] tracking-wider font-semibold ${isError ? 'text-danger' : 'text-text-muted'}`}>
        ...
        {expanded && (
          <pre className="mt-2 text-[11px] text-text leading-relaxed overflow-auto rounded-lg p-4"
            style={{ background: 'rgba(0,0,0,0.4)', whiteSpace: 'pre-wrap', wordBreak: 'break-all', maxHeight: '200px' }}>
            {content}
          </pre>
        )}
      </div>
...
function SubagentBlockWide({ type, prompt, id }: { type: string; prompt: string; id: string }) {
  ...
  return (
    <div
      className="mx-6 my-2 rounded-lg cursor-pointer"
      style={{ border: '1px dashed rgba(0,229,255,0.3)', background: 'rgba(0,229,255,0.03)' }}
      onClick={() => setExpanded(!expanded)}
    >
      <div className="px-5 py-3">
        <div className="flex items-center gap-2">
        ...
        {!expanded && prompt && (
          <div className="text-[12px] text-text-muted mt-1.5 truncate">{prompt.slice(0, 100)}</div>
        )}
        {expanded && (
...
function GapIndicator({ timestamp, gapMs }: { timestamp: string; gapMs: number }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2 px-6">
      ...
      <span className="text-[10px] text-text-muted tracking-wider flex-shrink-0">
